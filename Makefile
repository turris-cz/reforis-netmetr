...
lint-web: venv
	$(VENV_BIN)/$(PYTHON) -m pylint --rcfile=pylintrc reforis_netmetr
	$(VENV_BIN)/$(PYTHON) -m pycodestyle --config=pycodestyle reforis_netmetr
...
create-messages:
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_netmetr/translations/messages.pot .
init-langs: create-messages
	for lang in $(LANGS); do \
		$(VENV_BIN)/pybabel init \
		-i reforis_netmetr/translations/messages.pot \
		-d reforis_netmetr/translations/ -l $$lang \
	; done
update-messages:
	$(VENV_BIN)/pybabel update -i ./reforis_netmetr/translations/messages.pot -d ./reforis_netmetr/translations
compile-messages:
	$(VENV_BIN)/pybabel compile -f -d ./reforis_netmetr/translations

clean:
	find . -name '*.pyc' -exec rm -f {} +
	rm -rf $(VENV_NAME) *.eggs *.egg-info dist build .cache
	rm -rf dist build *.egg-info
	rm -rf $(JS_DIR)/node_modules/ reforis_static/reforis_netmetr/js/app.min.js
	$(PYTHON) -m pip uninstall -y reforis_netmetr
