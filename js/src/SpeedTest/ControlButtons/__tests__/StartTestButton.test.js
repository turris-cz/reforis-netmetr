...
describe("<StartTestButton />", () => {
    ...
    beforeEach(async () => {
        ({container, asFragment, getByText} = render(
            <StartTestButton ws={webSockets} setAsyncId={setAsyncId} asyncId={"test-async-id"}/>
        ));
    ...
    it("Should trigger redownload data on click.", () => {
        ...
        expect(mockAxios.post)
            .toHaveBeenCalledWith("/reforis/netmetr/api/trigger-measure-speed-and-download-data", undefined, expect.anything());
    });
